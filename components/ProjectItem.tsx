...
const ProjectItem = ({ title, description, date, link, image, madeWith}: ProjectItemProps) => {
  return (
    <ProjectLayout>
      ...
      <section className="flex flex-col md:flex-row justify-between w-full">
        <div className="flex flex-col justify-between">
          ...
          <section className="flex items-center w-auto p-4 bg-[#181818] my-5 md:my-0 rounded">
            <span className="font-thin text-gray-300 text-sm mr-3">MADE WITH:</span>
            {
              madeWith.map(tool => (
                <p key={`made-${tool}`} className='font-thin text-gray-300 text-sm italic mr-2'>{tool}</p>
              ))
            }
          </section>
        </div>
